...
def _invoke_worker(cmd: str, payload: dict[str, Any], *, timeout: float | None = _EXTRACT_TIMEOUT) -> Any:
    if not _use_subprocess():
        ...
        if cmd == "ping":
            return "pong"
        if cmd == "extract":
            return core.extract_info_inproc(str(payload["url"]))
        if cmd == "expand":
            return core.expand_playlist_inproc(str(payload["url"]))
        if cmd == "formats":
            return core.list_formats_inproc(str(payload["url"]))
        if cmd == "qualities":
            return core.list_video_qualities_inproc(str(payload["url"]))
        if cmd == "stream":
            fmt = payload.get("format_id")
            return core.get_stream_url_inproc(str(payload["url"]), str(fmt) if fmt else None)
...
def expand_playlist_urls(url: str) -> list[str]:
    ...
    got = _invoke_worker("expand", {"url": url})
...
def extract_info(url: str) -> dict[str, Any]:
    ...
    info = _invoke_worker("extract", {"url": url})
...
def get_format_stream_url(url: str, format_id: str) -> str:
    ...
    got = _invoke_worker("stream", {"url": url, "format_id": str(format_id)})
    if not got:
