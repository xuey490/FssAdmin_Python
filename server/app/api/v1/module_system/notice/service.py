...
class NoticeService:
    ...
    async def panel_data(self) -> PanelDataOut:
        ...
        messages = []
        try:
            from app.api.v1.module_system.logs.model import OperLogModel

            stmt = select(OperLogModel).where(OperLogModel.delete_time.is_(None)).order_by(desc(OperLogModel.create_time)).limit(5)
            result = await self.auth.db.execute(stmt)
            ...
            for log_entry in logs:
                messages.append(
                    PanelMessageItem(
                        id=log_entry.id,
                        title=log_entry.router or "系统操作",
                        content=f"{log_entry.method} {log_entry.router}",
                        time=log_entry.create_time.strftime("%Y-%m-%d %H:%M") if log_entry.create_time else "",
                        type="system",
                    )
                )
