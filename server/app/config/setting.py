...
class Settings(BaseSettings):
    ...
    # env_file 不在类定义时写死：get_settings() 按当前 ENVIRONMENT 动态传入
    ...
    SERVER_PORT: int = 8181  # 服务端口（修改 .env.dev 中的值即可变更）

    # ================================================= #
    # ******************* API文档配置 ****************** #
    # ================================================= #
    DEBUG: bool = True  # 调试模式
    DEMO_ENABLE: bool = False  # 演示模式：禁止改数据（仅放行 GET 与登录/刷新/登出）
    TITLE: str = "FssAdmin"  # 文档标题
    ...
    ALLOWED_ORIGINS: Annotated[list[str], NoDecode] = ["*"]  # 允许的域名列表（env 可用逗号分隔）
    ALLOW_METHODS: list[str] = ["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"]
    ALLOW_HEADERS: list[str] = ["Content-Type", "Authorization", "X-Requested-With", "X-Tenant-Id", "X-Correlation-ID"]
    ALLOW_CREDENTIALS: bool = True  # 是否允许携带cookie
    CORS_EXPOSE_HEADERS: list[str] = ["X-Request-ID", "X-Correlation-ID", "Content-Disposition"]
    CORS_MAX_AGE: int = 600  # 预检缓存秒数
    ...
    ACCESS_TOKEN_EXPIRE_SECONDS: int = 60 * 60 * 2  # access_token过期时间(秒)2 小时
    ...
    # jwt: JWT access/refresh + Redis tokenVersion/会话；redis-token: 不透明 token 全存 Redis
    SESSION_TYPE: Literal["jwt", "redis-token"] = "jwt"

    @field_validator("SESSION_TYPE", mode="before")
    @classmethod
    def _strip_session_type(cls, v: Any) -> Any:
    ...
    # 站点 URL（用于生成支付通知 URL）
    SITE_URL: str = "http://localhost:8181"

    # ================================================= #
    # ******************** 数据库配置 ******************* #
    # ================================================= #
    SQL_DB_ENABLE: bool = True  # 是否启用数据库
    DATABASE_ECHO: bool | Literal["debug"] = False  # sqlalchemy 原生 echo（一般勿开，用下面两项）
    # ORM SQL 回显（推荐）：互不影响，可同时开
    SQL_ECHO_CONSOLE: bool = False  # 控制台绿色打印每条 SQL
    SQL_ECHO_FILE: bool = False  # 按天写入 logs/sql/sql_YYYY-MM-DD.log
    ...
    # ***************** 视频下载配置 ***************** #
    # ================================================= #
    VIDEO_DOWNLOAD_ROOT: Path = Path("static/videos")  # 视频下载根目录
    VIDEO_DOWNLOAD_WORKERS: int = 2  # 并发下载工人数
    ...
    # ******************* 请求限制配置 ****************** #
    # ================================================= #
    # IP 全局限流总开关（默认关；开启后单 IP 每分钟最多 RATE_LIMIT_TIMES 次）
    RATE_LIMIT_ENABLED: bool = False
    RATE_LIMIT_TIMES: int = 10000
    RATE_LIMIT_SECONDS: int = 60
    REQUEST_LIMITER_REDIS_PREFIX: str = "fssadmin:request_limiter:"

    # APScheduler 启动开关（需 Redis；关闭时不 init_scheduler）
    ...
    def MIDDLEWARE_LIST(self) -> list[str | None]:
        ...
        # 中间件执行顺序（从外到内）：
        #   CORS → RequestLog → GZip → CorrelationId → Tenant → OperationLog → 业务路由
        ...
        MIDDLEWARES: list[str | None] = [
            ...
            # 最内层：业务依赖写完 request.state.auth 后再落操作日志（对齐 phpserver）
            "app.core.middlewares.OperationLogMiddleware" if self.OPERATION_LOG_RECORD else None,
        ]
        return MIDDLEWARES
...
def reload_settings() -> Settings:
    """清除缓存并刷新模块级 settings（CLI 切换 --env 后调用）。"""
