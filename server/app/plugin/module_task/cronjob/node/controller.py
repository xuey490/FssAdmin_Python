...
async def get_node_job_logs_controller(
    id: Annotated[int, Path(description="节点ID")],
    page: Annotated[PaginationQueryParam, Depends()],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_task:cronjob:node:query"]))],
    status: Annotated[int | None, Query(description="执行状态", ge=0, le=5)] = None,
    trigger_type: Annotated[str | None, Query(description="触发方式")] = None,
) -> JSONResponse:
    result_dict = await NodeService(auth).job_logs_page(
        node_id=id,
        page_no=page.page_no,
        page_size=page.page_size,
        status=status,
    ...
    return SuccessResponse(data=result_dict, msg="查询节点执行日志成功")
