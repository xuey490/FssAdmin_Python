...
class NodeService:
    ...
    async def job_logs_page(
        ...
        """按节点 ID 查 task_job（调度器 job_id == 节点 id）。"""
        from app.common.enums import QueueEnum
        from app.plugin.module_task.cronjob.job.crud import JobCRUD
        from app.plugin.module_task.cronjob.job.schema import JobOutSchema

        obj = await NodeCRUD(self.auth).get_obj_by_id_crud(id=node_id)
        if not obj:
            raise CustomException(msg="节点不存在")

        search: dict = {"job_id": (QueueEnum.eq.value, str(node_id))}
        if isinstance(status, int):
            search["status"] = (QueueEnum.eq.value, status)
        if trigger_type:
            search["trigger_type"] = (QueueEnum.eq.value, trigger_type)
