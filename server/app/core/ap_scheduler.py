...
async def cleanup_operation_log() -> None:
    """系统任务：每周软删 30 天前操作日志（须模块级函数，Redis JobStore 才能序列化）。"""
    ...
    async with async_db_session() as session:
        async with session.begin():
            result = await session.execute(
                sa_update(OperLogModel)
                .where(OperLogModel.delete_time.is_(None))
                .where(OperLogModel.create_time < cutoff)
                .values(delete_time=datetime.now())
            )
...
class SchedulerUtil:
    ...
    def _register_system_jobs(cls) -> None:
        ...
        def _add(job_id: str, name: str, func, trigger) -> None:
            nonlocal registered
            try:
                scheduler.add_job(
                    func,
    ...
    def _update_job_log(
        ...
        with Session(engine) as session:
            job_log = (
                ...
                .query(JobModel)
                .filter(JobModel.job_id == job_id, JobModel.status == JOB_STATUS_PENDING)
                .order_by(JobModel.create_time.desc())
                .first()
            )
            if job_log:
