...
class CustomOAuth2PasswordBearer(OAuth2PasswordBearer):
    ...
    async def __call__(self, request: Request) -> str | None:
        ...
        if not authorization or scheme.lower() != settings.TOKEN_TYPE.lower():
            # 媒体播放器等无法带 Header 时，允许 ?access_token=
            q = request.query_params.get("access_token") or request.query_params.get("token")
            if q:
                return q
            if self.auto_error:
                raise CustomException(msg="认证失败,请登录后再试", code=401, status_code=401)
            return None
        return token
...
def decode_access_token(token: str) -> JWTPayloadSchema:
    ...
    """
    if not token:
        raise CustomException(msg="认证不存在,请重新登录", code=401, status_code=401)
    ...
    try:
        ...
        if not online_user_info:
            raise CustomException(msg="无效认证,请重新登录", code=401, status_code=401)
        ...
        raise CustomException(msg="认证已过期,请重新登录", code=401, status_code=401)

    except jwt.InvalidTokenError:
        raise CustomException(msg="token已失效,请重新登录", code=401, status_code=401)
