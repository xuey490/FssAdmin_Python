...
class CRUDBase[ModelType: MappedBase, CreateSchemaType: BaseModel, UpdateSchemaType: BaseModel]:
    ...
    async def update(self, id: int, data: UpdateSchemaType) -> ModelType:
        ...
        try:
            ...
            # 租户归属检查（超管也不得跨租户改业务数据；跨租户请显式 with_ignore_tenant）
            if self.auth and self.auth.user and self.auth.tenant_id:
                if hasattr(obj, "tenant_id"):
                    obj_tid = getattr(obj, "tenant_id", None)
                    if obj_tid is not None and int(obj_tid) != int(self.auth.tenant_id):
                        is_platform = getattr(self.model, "__platform_data_shared__", False)
                        if is_platform and int(obj_tid) == 1:
    ...
    def _platform_shared_conditions(self) -> list[ColumnElement]:
        ...
        if tid is not None and int(tid) != 1:
            return [
                (getattr(self.model, "tenant_id") == int(tid))
                | (getattr(self.model, "tenant_id") == 1)
            ]
        return []

    def _tenant_dml_where(self, sql):
        """为 DML 语句注入 tenant_id 条件（超管同样隔离）。"""
        if self.auth and hasattr(self.model, "tenant_id") and self.auth.user:
            tid = self.auth.tenant_id
            if tid is not None and int(tid) > 0:
                return sql.where(getattr(self.model, "tenant_id") == int(tid))
        return sql
        ...
        if self.auth and hasattr(self.model, "tenant_id") \
           and not getattr(self.model, "__platform_data_shared__", False):
            if self.auth.user:
                tid = self.auth.tenant_id
                if tid is not None and int(tid) > 0:
                    conditions.append(getattr(self.model, "tenant_id") == int(tid))
