...
export function setupBeforeEachGuard(router: Router): void {
  ...
  router.beforeEach(
    async (
      ...
      return handleRouteGuard(to, router)
    }
  )
}
...
/**
 * 处理登录状态
 * @returns true 表示可以继续，RouteLocationRaw 表示已处理跳转
 */
...
async function handleDynamicRoutes(
  ...
  try {
    ...
    if (!menuProcessor.validateMenuList(menuList)) {
      // 菜单为空：注册默认 dashboard 路由，跳转到 console 首页
      console.warn('[RouteGuard] 菜单列表为空，用户暂无菜单权限，跳转默认首页')
      const defaultMenuList = [dashboardRoutes]
      routeRegistry?.register(defaultMenuList)
      const menuStore = useMenuStore()
      menuStore.setMenuList(defaultMenuList)
      menuStore.addRemoveRouteFns(routeRegistry?.getRemoveRouteFns() || [])
      routeInitInProgress = false
      closeLoading()
      return { path: '/dashboard/console', replace: true }
    }
...
/**
 * 处理根路径重定向到首页
 * @returns false 表示无需跳转，RouteLocationRaw 表示已处理跳转
 */
