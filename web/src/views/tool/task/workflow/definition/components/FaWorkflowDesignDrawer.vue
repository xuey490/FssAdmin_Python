...
import type { FormExpose, FormItem } from "@/types/form-item";
...
const loadNodeTypes = async () => {
  loading.value = true;
  try {
    const list = (await WorkflowNodeTypeAPI.getWorkflowNodeTypeOptions()) || [];
    allNodes.value = list.map((nodeType) => ({
      id: nodeType.id,
      type: nodeType.code,
      name: nodeType.name,
...
onInit((vueFlowInstance) => {
  ...
  if (workflowId.value) {
    (async () => {
      try {
        const detail = await WorkflowDefinitionAPI.getWorkflowDetail(workflowId.value!);
...
function handleSave() {
  ...
  if (workflowId.value) {
    return WorkflowDefinitionAPI.updateWorkflow(workflowId.value, saveData);
  } else {
    return (async () => {
      const created = await WorkflowDefinitionAPI.createWorkflow(saveData);
      if (created?.id) {
