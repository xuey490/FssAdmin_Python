...
export function useChart(options: UseChartOptions = {}) {
  ...
  // 等待容器具有有效尺寸后执行回调，最多重试 maxRetries 次
  const waitForDimensions = (el: HTMLElement, callback: () => void, maxRetries = 10) => {
    let retries = 0
    const check = () => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && pendingOptions && !isDestroyed) {
            ...
            // 等待容器尺寸就绪后再初始化，避免 clientWidth/clientHeight 为 0 的警告
            waitForDimensions(target, () => {
